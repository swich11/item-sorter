...
bool Planner::move(std::shared_ptr<interfaces::srv::Move::Response> res, bool grasp, const geometry_msgs::msg::Pose &goal) {
    ...
    grab_pose.position.z = GRAB_OFFSET + GRIPPER_HEIGHT;
    move_group_interface->setPoseTarget(grab_pose);
    move_group_interface->stop();
    // setGripPathConstraints();
    auto ret = move_group_interface->plan(plan);
    if (ret != moveit::core::MoveItErrorCode::SUCCESS) {
        RCLCPP_ERROR(this->get_logger(), "Planning Failed :(.");
        res->success = false;
